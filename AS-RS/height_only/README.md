...
1. reorganize 究竟要不要把所有貨物往兩邊放？另外，已經在原位的貨物還有需要移動嗎？還是就下移就好了？
2. remove item需不需要把上面的東西往下移？一來這樣可以多用到一些空間，二來要放東西的時候就可以直接放入。若reorganize 不打算移動所有貨物，並在放入貨物時哪裡可以放就先放哪裡，remove item就必須把上面的東西下移。反之按照原想法的話，不移動也沒關係。
3. 空棧板都往出入口兩邊堆疊
4. ~~要怎麼表達空棧板？~~(已解決)
5. 需要寫一個吐出所有物品在哪裡的function
6. ~~要輸出離出口最近的托盤~~(已解決)
7. 確認整體流程是哪個：
    - 放貨物 -> 開放取貨 -> 重新整理 -> 放貨物 （每次放貨物前都會重新整理）
    - 放貨物 -> 開放取貨 -> 放貨物 -> 重新整理 （每天到一個時間才會重新整理）
8. 是否要最小化重新整理的移動次數？還是要以空間利用率為優先？
9. 取出離出口最近的托盤後，應該把上面的托盤往下挪？或是放回去時，不能只看最高的那個是多少，應該要找到空位插進去
10. API是否要有記憶性？重新整理時是否需要回傳原位置和新位置，還是只要回傳新位置即可？
11. Best fit是否需要把所貨物進行重整？還是只要把新的貨物重新整理就好？
12. 可能需要新增一個一次放入一堆東西的api？
