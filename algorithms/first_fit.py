...
def first_fit(item_to_place:Item, all_bins, online_priority:list, bin_dimensions:tuple, best_pallet: Item):
    ...
    :return: A dictionary containing the placement details:
             - 'pallet_id': ID of the pallet being placed.
             - 'original_pallet_placed_bin': ID of the bin where the pallet was originally placed.
             - 'original_pallet_position': Position of the pallet in the original bin.
             - 'target_bin': ID of the bin where the item will be placed.
             - 'target_position': Position in the target bin where the item will be placed.
    :raises ValueError: If the item cannot be placed due to dimension constraints or if no suitable bin is found.
    """

    item_dimension = utils.get_optimal_dimension(item_to_place, bin_dimensions)
    
    if item_dimension is None:
        raise ValueError (f"Item {item_to_place.pallet_id} cannot be placed due to dimension constraints.")
    ...
    for bin_id in online_priority:
        ...
        if bin.can_place(item_to_place):
            ...
            break
    if best_bin is None:
        raise ValueError(f"Item {item_to_place.pallet_id} cannot be placed in any bin due to running out of space.")

    return {
        'pallet_id': best_pallet.pallet_id if best_pallet else None,
