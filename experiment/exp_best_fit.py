...
def exp_best_fit(items:list, all_bins, bin_dimensions, offline_priority=None):
    """
    ...
    :param all_bins: A dictionary of Bin objects representing available bins, excluding bins for pallets.
    :param bin_dimensions: A tuple representing the dimensions
                            (width, height, depth, min_adjust_length) of the bins.
    :param offline_priority: A list of bin IDs in the order of priority for placement.
    :return: list of unplaced Item objects).
    """
    # put down all the items in the bins if needed. The codes for this part has not been implemented yet.
    # for bin in all_bins.values():
    #     sorted_items = sorted(items, key=lambda item: item.position[1])
    #     bin.reset()
    #     for item in sorted_items:
    #         new_position = (0, bin.get_current_height(), 0)
    #         bin.place_item(item, new_position)

    if offline_priority is None:
        raise ValueError("offline_priority must be provided for Best Fit algorithm")
    ...
    items.sort(key=lambda i: i.placed_dimensions[1], reverse=False)
